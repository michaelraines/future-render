...
// Models an ID3D12GraphicsCommandList.
type Encoder struct {
	inner backend.CommandEncoder
}

// BeginRenderPass begins a render pass (OMSetRenderTargets + ClearRenderTargetView).
...
// SetPipeline binds a PSO (SetPipelineState + SetGraphicsRootSignature).
...
// SetVertexBuffer binds a vertex buffer (IASetVertexBuffers).
...
// SetIndexBuffer binds an index buffer (IASetIndexBuffer).
...
// SetTexture binds a texture via descriptor table (SetGraphicsRootDescriptorTable).
...
// Flush submits the command list (ExecuteCommandLists + Signal fence).
