...
// Models an MTLRenderCommandEncoder.
type Encoder struct {
...
// SetPipeline binds a render pipeline (setRenderPipelineState:).
...
// SetVertexBuffer binds a vertex buffer (setVertexBuffer:offset:atIndex:).
...
// SetTexture binds a texture (setFragmentTexture:atIndex:).
...
// Flush submits all recorded commands (commit).
